...
public final class Table {
    ...
    public Table() {
        ...
        this.gameFrame = new JFrame("Chess - Hot Seat Mode (PvP)");
    ...
    void handleMoveMade(final Move move, final PlayerType playerType) {
        // IMPORTANT: Flip the board after every move for Hot Seat mode
    ...
    void handleNewGame() {
        undoAllMoves();
        this.chessBoard = Board.createStandardBoard();
        // Reset direction to Normal for white to start
        this.boardDirection = BoardDirection.NORMAL;
        this.show();
    ...
    private void undoLastMove() {
        ...
        if(!log.isEmpty()) {
            final Move lastMove = Table.get().getMoveLog().removeMove(Table.get().getMoveLog().size() - 1);
            this.chessBoard = this.chessBoard.currentPlayer().unMakeMove(lastMove).getToBoard();
            // When undoing, flip board back to previous player
            this.boardDirection = this.boardDirection.opposite();
                ...
                public void mouseClicked(final MouseEvent event) {
                    ...
                    if (isRightMouseButton(event)) {
                        ...
                        if (sourceTile == null) {
                            ...
                            // Ensure we pick our own piece
                            if (humanMovedPiece == null ||
                                    humanMovedPiece.getPieceAllegiance() != chessBoard.currentPlayer().getAlliance()) {
                                sourceTile = null;
                                humanMovedPiece = null;
                            }
                        } else {
